...
z3::expr getPathCondition(z3::context &Z3, BasicBlock &From, BasicBlock &To, 
  ...
  if(BranchInst* Br = dyn_cast<BranchInst>(Term)) {
    if(Br->isUnconditional()) {
      ...
      BrCond = Z3.bool_val(true);
    } else {
      Value* Cond      = Br->getCondition();
        // se puede hacer getSuccessor[0] para el true y 1 para el false
      // Value* CondTrue  = Br->getTrueValue();
      // Value* CondFalse = Br->getFalseValue();

      // TODO If it is a conditional branch, depends if B is the true branch or the false branch (or both !) 
      // Preguntar: La condicion es un bloque especial, que significa que tenga true y false value?
      // BasicBlock* CondTrueBlock  = dyn_cast<BasicBlock>(CondTrue);
      // BasicBlock* CondFalseBlock = dyn_cast<BasicBlock>(CondFalse);
      
      // Esto hay que armarlo? como?
      // Algo hay que tomar del valor de cond?
      // z3::expr ExprCondicionTrue  = ???
      // z3::expr ExprCondicionFalse = ???
      
      // Casos:
      //   1) B es directo a la condicion -> depende de la condicion -> trueVal == B
      //   2) B esta al final -> es el cierre de la condicion
      // if (&To == CondTrueBlock) {
      //   // Caso 1: true
      //   BrCond = BrCond && ExprCondicionTrue;

      //   // Agrego la instruccion al mapa de instruccion a condicion
      //   Inst2Cond.insert(CondTrue, ExprCondicionTrue);

      // } else if (&To == CondTrueBlock) {
      //   // Caso 1: false
      //   BrCond = BrCond && ExprCondicionFalse;

      //   // Agrego la instruccion al mapa de instruccion a condicion
      //   Inst2Cond.insert(CondFalse, ExprCondicionFalse);
      // } else {
      //   // Caso 2
      //   BrCond = BrCond && (ExprCondicionTrue || ExprCondicionFalse);
      ...
      // add target library pack en cmake y pasarle z3_libraries
